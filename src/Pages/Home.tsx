...
import { testAction } from '../reducers/testSlice'
import { RootState } from '../app/rootReducer'

const Home: React.FC = () => {
  const testText: string = useSelector(({ test }: RootState) => test.text)
  const dispatch = useDispatch()
  const test = () => dispatch(testAction())
  return (
    <div>
      <p>Hello,</p>
      <p>
      ...
      <p>
        ...
        . You should see the message change here: {testText}.
      </p>
      <p>
        This React app imitates a website with articles or posts. On the left
        you have a menu to navigate between this page ("Home"), page that lists
        all the post ("Posts"), and the page that adds posts ("Add posts").
      </p>
      <p>
        Also, to complete this test you will require an API. You can start the
        API by running <code>yarn api</code> or <code>npm run api</code>. To use
        the api you will make a <code>GET</code> request to{' '}
      ...
      <ul>
        ...
        <li>
          Edit the page Header to display the total count of posts. (Anywhere in
          the header is fine)
        </li>
        <li>
          Cover project with tests, using jest & enzyme, or your favourite
          libraries
        </li>
      </ul>
      <p>
      ...
      <ul>
        <li>Convert project to TypeScript</li>
      </ul>
      <p>
        Once finished, push to you repo on github, and sent us a link without
